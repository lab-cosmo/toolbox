...
void banner()
{
    std::cerr
            ...
            << " -ipdb     input is an pdb format. velocities might be read as 4,5,6 field      \n"
            ...
            << " -posmax    maximum distance to compute rho(z) [5]                              \n"
            << " -posbins   number of bins in the histogram for rho(z) [100]                    \n"
...
int main(int argc, char **argv)
{
    ...
    double cogdr, copos, dt, densw, pdmax, p3dmax, hwinfac; unsigned long fstart,fstop,fstep,gdrbins, posbins, vvlag, msdlag, ftpad, dbinsx, dbinsy, dbinsz, dfoldx, dfoldy, dfoldz, cvtype, pdbins, p3dbins;
    ...
    bool fok=
            ...
            clp.getoption(copos,"posmax",8.) &&
            clp.getoption(posbins,"posbins",(unsigned long)100) &&
            //vvacf options
    ...
    Histogram<double> hpos(0.,copos, posbins);
    hpos.get_options(hgwo);

    double d12, dx, dy, dz, rhoz, rho, cog2=cogdr*cogdr, copos2=copos*copos, ngdrpairs=0, npospairs=0;
    int nfr=0, npfr=0;
    ...
    std::ostream *ogdr, *opos, *orho, *oxtc2xyz, *ocv, *ovvac, *omsd, *odipole, *odens, *odtraj, *odpov, *opca, *opcaxyz, *opd, *op3d, *opproj, *otherm, *ocharge;
    std::cout.precision(8); std::cout.width(15); std::cout.setf(std::ios::scientific);

    if (prefix=="")
    {
        //everything goes to stdout
        ocv=ogdr=opos=oxtc2xyz=ovvac=omsd=orho=odipole=odens=odtraj=opca=opcaxyz=opd=op3d=opproj=otherm=ocharge=&std::cout;
    }
    else
    {
        if (fgdr)
        ...
        if (fpos)
        {
            ...
            orho=new std::ofstream((prefix+std::string(".rhoz")).c_str());
            (*orho).precision(8); (*orho).width(15); (*orho).setf(std::ios::scientific);
        }
        if (fxtc2xyz)
        {
    ...
    ffirstcell=true;    
    while ((fdlp && ReadDLPFrame(std::cin,af))||(fxtc && ReadXTCFrame(stdin,af)) || (fxyz && ReadXYZFrame(std::cin,af)) || (fpdb && ReadPDBFrame(std::cin,af)) )
    {
        ++nfr; 
        ...
        { 
            ...
            for( int i=0 ; i<af.ats.size(); ++i ) {af.ats[i].name=reffr.ats[i].name; af.ats[i].nprops["mol"]=reffr.ats[i].nprops["mol"];}      
        ...
        std::vector<double> dlthetax, dlthetay, dlthetaz; 
        ...
        if (fpos)
        {
            ...
            {                
                ...
                rhoz=dlz[j]*dlz[j];
                if (rhoz>copos2||rhoz==0.) continue;
                hpos.add(sqrt(rhoz),statweight);  // this has the possibility of being weighted
            }
            npospairs+=al2.size()*statweight;   // potential number of reference atoms in the cell volume
            
               
            for (unsigned long j=0; j<al2.ats.size(); ++j)
            {            
                int k=0;
                dlthetax.resize(al2.size());
                dlthetay.resize(al2.size());
                dlthetaz.resize(al2.size());
                for (unsigned long i=0; i<af.ats.size(); ++i)
                { 
                    if(af.ats[i].nprops["mol"]==al2.ats[j].nprops["mol"] && af.ats[i].molname==al2.ats[j].molname && af.ats[i].group==al2.ats[j].group)
                    {
                        if(af.ats[i].name=="C" && af.ats[i].molname=="MEO")  //calculation of the angle between the CO axis and the normal axis to graphite planes 
                        {
                            dlthetax= al2[j].x-af[i].x;
                            dlthetay= al2[j].y-af[i].y;
                            dlthetaz= al2[j].x-af[i].z;
                        }
                        if(af.ats[i].name=="H" && af.ats[i].molname=="TP3M") //calculation of the angle between the dipole axis and the normal axis to graphite planes
                        {
                            dlthetax= dlthetax + (al2[j].x-af[i].x);
                            dlthetay= dlthetay + (al2[j].y-af[i].y);
                            dlthetaz= dlthetaz + (al2[j].z-af[i].z);
                        }
                    }
                }
        ...
        if (fxtc2xyz)
        {
                ...
                for (unsigned long i=0; i<af.ats.size(); ++i)
                {
                    (*oxtc2xyz)<<af.ats[i].name<<" "<<af.ats[i].nprops["mol"]<<" "<<af.ats[i].x<<" "<<af.ats[i].y<<" "<<af.ats[i].z<<" "<<std::endl;
                }
        }
    ...
    if (fpos)
    {
        ...
        std::cerr<<"# PRINTING OUT rho(z) OF "<<lrat<<" THAN "<<ltype<<" REFERENCE\n";
        std::valarray<double> z, w, rho;
        hpos.get_bins(z, w, rho);
        for (unsigned long i=0; i<z.size(); ++i)
        { rho[i]=rho[i]/(4*constant::pi*z[i]*z[i]); }
        //!!CHECK NORMALIZATION IN CASE SAME SPECIES ARE USED!!
        if (cvolume == 0.) rho*=4./3.*constant::pi*copos*copos*copos;
        else rho*=hpos.samples()/(npospairs/cvolume);
        for (unsigned long i=0; i<z.size(); ++i)
           (*orho)<<z[i]<<" "<<rho[i]<<std::endl;
    }
    if (fpd)
    {
